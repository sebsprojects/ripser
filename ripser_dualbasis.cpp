...
void compute_dualbasis(ripser& ripser,
                       ...
                       index_t dim)
{
	...
	index_t n = columns_to_reduce.size() + cleared_columns.size();
	std::vector<index_t> parities(n, 0);
	std::vector<index_t> offs(n, 0);
	...
	std::vector<std::vector<index_diameter_t>> Q;
	Column P_j;
	std::cout << "Dual basis comp with: " << columns_to_reduce.size() << " " << cleared_columns.size() << std::endl;
	      ...
	      cc_index < (index_t) cleared_columns.size())
	{
			...
			if(pair != pivot_column_index_dimm1.end()) {
				...
				if(V_dimm1.column_start(pivot_index) != V_dimm1.column_end(pivot_index)) {
						...
						l--)
					{
						...
						add_partial_simplex_coboundary(ripser, v_ele, P_j, dim - 1);
					}
				}
		...
		// COMPUTE PRODUCT
		Q.push_back(std::vector<index_diameter_t>());
		Q.at(ctr_index + cc_index - 1).push_back(current_simplex);
		index_t num_pops = 0;
		index_diameter_t v_ele = pop_pivot(P_j);
		if(get_index(v_ele) != -1) {
			while(get_index(v_ele) != -1) {
				for(index_t k = 0; k < ctr_index + cc_index; k++) {
					std::vector<index_diameter_t>& Q_k = Q.at(k);
					// Still elements in rep left to work with?
					index_t rep_index = ((index_t) Q_k.size()) - 1 - offs[k];
					while(rep_index >= 0) {
						index_diameter_t rep_ele = Q_k.at(rep_index);
		...
		for(index_t k = 0; k < ctr_index + cc_index; k++) {
			if(parities.at(k) % 2 == 1) {
				Q.at(k).push_back(current_simplex);
			}
			parities[k] = 0;
			offs[k] = 0;
		}
	}
	for(index_t k = 0; k < (index_t) Q.size(); k++) {
		std::vector<index_diameter_t> Q_k = Q.at(k);
		std::cout << "k=" << k << ": ";
		for(index_diameter_t& q_ele : Q_k) {
		  std::cout << simplex_tos(ripser, q_ele, dim, 0, 0, 1, 0, 0);
			if(q_ele != Q_k.back()) {
				std::cout << " :: ";
			}
		}
