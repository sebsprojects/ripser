...
def plot_rep(ax, samples, rep, s=False):
    if not s:
        x, y = zip(*samples[30:])
...
hh = barskip
for b in intervals[1][1][1:]:
    c = colors[0]
    rect = pat.Rectangle((b[0], hh / h), b[1] - b[0], barh / h, linewidth=0, facecolor=c)
...
for i in range(12):
    ...
    init_plotax(ax)
    plot_rep(ax, data, reps[1][1][i], i == 0)
    x += repw + marginw
    if i == 5:
        x = marginw
